...
var PROXY_PORT = process.env.SPLUNK_PROXY_PORT || 80;
...
var server = http.createServer(function (req, res) {
    ...
    var headers = {
        ...
        'User-Agent': 'Splunk Example Proxy'
    };
    var requestHeaders = {};
    for (var i = 0; i + 1 < req.rawHeaders.length; i += 2) {
        ...
        if (key.toLowerCase() === 'splunk-access-token') {
            accessToken = value;
        }
    }
    ...
    if (!tracer) {
        tracer = new splunktracing.Tracer({
            access_token   : accessToken,
            component_name : 'splunk-tracer/examples/node_proxy',
        });
